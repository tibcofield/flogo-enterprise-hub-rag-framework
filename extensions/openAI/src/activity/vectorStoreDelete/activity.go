...
var logger = log.ChildLogger(log.RootLogger(), "openai-vector-store-delete")
...
// Activity is an OpenAI Vector Store Delete activity.
...
func (a *Activity) Eval(ctx activity.Context) (done bool, err error) {
	logger.Infof("****************** Starting OpenAI Vector Store Delete Activity Eval ******************")
	...
	logger.Infof("Deleting vector store with id '%s'", input.VectorStoreID)

	// Call the OpenAI API
	deleted, err := a.oaiClient.VectorStores.Delete(clientCtx, input.VectorStoreID)
	if err != nil {
		// Check for timeout specifically
		if clientCtx.Err() == context.DeadlineExceeded {
			contextErr := fmt.Errorf("request timeout: deleting vector store '%s' exceeded %d seconds",
				input.VectorStoreID, input.TimeoutSeconds)
			logger.Error(contextErr.Error())
			return false, contextErr
		}
		contextErr := fmt.Errorf("OpenAI API error: failed to delete vector store '%s' at endpoint '%s': %w",
	...
	logger.Infof("Successfully deleted vector store with id '%s' (deleted=%t)", out.ID, out.Deleted)
